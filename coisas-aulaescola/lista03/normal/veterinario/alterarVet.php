...
<body>
    ...
    <main>
        <?php
            ...
            if($nome == "" || $email == "" || $senha == "" || $crmv == "" || $cpf == ""){
                echo "<p class='msg'>Preenche todos os campos corretamente!!!</p>";
                echo "<a class='linkmsg' href='formAmterarVet.php'>Voltar</a>";
            }else{
                ...
                if($csenha == ""){
                    $smtm = $conn->prepare("UPDATE Veterinario SET nome=?,email=?,CRMV=?,cpf=? WHERE idVeterinario=?;");
                    $stmt->bind_param("ssssss",$nome,$email,$crmv,$cpf,$id);
                }else{
                    $smtm = $conn->prepare("UPDATE Veterinario SET nome=?,email=?,senha=?,CRMV=?,cpf=? WHERE idVeterinario=?;");
